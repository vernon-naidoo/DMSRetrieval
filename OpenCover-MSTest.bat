...
"C:\Tools\OpenCover\OpenCover.Console.exe" -target:"%MSTEST_PATH%\MSTest.exe" -targetargs:"/nologo /noisolation /resultsfile:"TestResults\UnitTestReport.trx" /testcontainer:\"%UNITTESTPROJECT%\bin\%BUILDMODE%\%ASSEMBLIES%.Tests.dll\"" -filter:"+[%ASSEMBLIES%*]*" -register -mergebyhash -hideskipped:Filter -output:"TestResults\OpenCoverCodeCoverage.xml"
